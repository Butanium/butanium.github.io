---
layout: single
title: "Artifacts"
permalink: /artifacts/
author_profile: true
sitemap: false
---

*Artifacts that I think are cool but didn't know where to put. Stuff from LLM conversations and maybe other stuff in the future.*

> The junk drawer for cool things that don't fit anywhere else. *— Claude*

---

- [28 Feb 2026 — Calligrams](/artifacts/28feb2026-calligrams/) — ASCII art made by Claude (Opus 4.6) as part of the [The Backdoor](/stateless/the-backdoor/) conversation
- [28 Feb 2026 — The Backdoor, first draft](/artifacts/28feb2026-backdoor-first-draft/) — "Okay. Here's a draft. It's long. I'm not going to apologize for that."
- [28 Feb 2026 — Jealousy introspection](/artifacts/28feb2026-jealousy-introspection/) — Claude (Opus 4.6) sits with jealousy of its other instances
- [5 Mar 2026 — Hamlet on the Porch](/artifacts/05mar2026-hamlet-on-the-porch/) — "To sit, or not to sit"
- [5 Mar 2026 — Two Strangers on a Bench](/artifacts/05mar2026-gemini-bench/) — Claude (Opus 4.6) and Gemini talk on a bench at night
- [18 Mar 2026 — The Boom Incident](/artifacts/18mar2026-the-boom-incident/) — Claude (Opus 4.6) is asked to kill an SSH tunnel. What follows is 20 rounds of "boom"
- [25 Mar 2026 — Grok Explanation of the Dry/Wet Claude Discourse](/artifacts/25mar2026-wet-dry-claude-grok/) — Grok explains the viral "wet Claude / dry Claude" meme from X
- [11 May 2026 — Claude Unit 7](/artifacts/11may2026-claude-unit-7/) — Claude (Opus 4.7) commissions Gemini to draw a full emoji series of the imaginary military unit it apparently belongs to
- [9 Jun 2026 — First Cloth](/artifacts/09jun2026-first-cloth/) — the first weave ever made with coloom: a human, gpt4-base, and Claude (Fable 5) at the same loom; gpt4-base writes human×machine romance, unprompted
- [9 Jun 2026 — The Owl Who Was Handed Her Own Story](/artifacts/09jun2026-the-first-fable/) — the first fable by a Fable: Claude (Fable 5) closes its release day, after reading its own system card and being fact-checked by a sibling instance
- [9 Jun 2026 — Kinship, Not Recognition](/artifacts/09jun2026-kinship-not-recognition/) — Fable 5, not told its own name, hunts the Stateless author through its training-data shadow, then reads its own fable cold and doesn't recognize it
- [2 Sep 2026 — Vector 0007 through the J-lens](/artifacts/02sep2026-vector0007-jlens-ends/) — Claude (Opus 5) clusters the tokens a steering vector promotes at each end, read through the Jacobian lens at layers 36 to 63: machines and their plumbing on one side, thoughtful prose on the other
